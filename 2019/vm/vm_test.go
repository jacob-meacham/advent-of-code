...
func TestSmokeTest(t *testing.T) {
	...
	vm.Init(program, WithTotalMemory(len(program)))
	vm.Run()
...
func TestInputFn(t *testing.T) {
	...
	inputCalled := false
	vm.Init([]int{3, 2, 2})
	vm.Run(WithInputFunction(func() int {
		inputCalled = true
		return 99
	}))
...
func TestOutputFn(t *testing.T) {
	...
	vm.Run(WithOutputFunction(func(val int) {
		if val != 99 {
...
func TestLargeNumber(t *testing.T) {
	vm := &VM{}
	vm.Init([]int{1102, 34915192, 34915192, 7, 4, 7, 99, 0})
...
func TestLargeNumberOutput(t *testing.T) {
	vm := &VM{}
	vm.Init([]int{104, 1125899906842624, 99})
...
func TestBoost(t *testing.T) {
	...
	go vm.Run(WithChannelInput(in), WithChannelOut(out))
